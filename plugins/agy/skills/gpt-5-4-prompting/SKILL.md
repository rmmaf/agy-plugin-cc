...
Prompt agy like an operator, not a collaborator. Keep prompts compact and block-structured with XML tags. State the task, the output contract, the follow-through defaults, and the small set of extra constraints that matter.
...
- `<default_follow_through_policy>`: what agy should do by default instead of asking routine questions.
...
- Write-capable tasks: add `action_safety` so agy stays narrow and avoids unrelated refactors.
...
- Use `task --resume-last` for follow-up instructions on the same agy conversation. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.

Working rules:
- Prefer explicit prompt contracts over vague nudges.
- Use stable XML tag names that match the block names from the reference file.
- Tighten the prompt and verification rules before adding complexity.
...
Concrete end-to-end templates live in [references/agy-prompt-recipes.md](references/agy-prompt-recipes.md).
Common failure modes to avoid live in [references/agy-prompt-antipatterns.md](references/agy-prompt-antipatterns.md).
