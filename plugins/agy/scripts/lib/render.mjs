...
function formatAntigravityResumeCommand(job) {
  if (!job?.threadId) {
    return null;
  }
...
function appendActiveJobsTable(lines, jobs) {
  ...
  lines.push("| Job | Kind | Status | Phase | Elapsed | Antigravity Session ID | Summary | Actions |");
  lines.push("| --- | --- | --- | --- | --- | --- | --- | --- |");
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.threadId) {
    lines.push(`  Antigravity session ID: ${job.threadId}`);
  }
  const resumeCommand = formatAntigravityResumeCommand(job);
  if (resumeCommand) {
    lines.push(`  Resume in Antigravity: ${resumeCommand}`);
  }
...
export function renderSetupReport(report) {
  const lines = [
    ...
    `- Antigravity: ${report.Antigravity.detail}`,
...
export function renderNativeReviewResult(result, meta) {
  ...
  if (stdout) {
    ...
    lines.push("Antigravity review failed.");
  }
...
export function renderStoredJobResult(job, storedJob) {
  ...
  if ((isStructuredReviewStoredResult(storedJob) || isResearchStoredResult(job, storedJob)) && storedJob?.rendered) {
    ...
    return `${output}\nAntigravity session ID: ${threadId}\nResume in Antigravity: ${resumeCommand}\n`;
  }

  const rawOutput =
    (typeof storedJob?.result?.rawOutput === "string" && storedJob.result.rawOutput) ||
    (typeof storedJob?.result?.Antigravity?.stdout === "string" && storedJob.result.Antigravity.stdout) ||
    "";
  if (rawOutput) {
  ...
  if (threadId) {
    lines.push(`Antigravity session ID: ${threadId}`);
    lines.push(`Resume in Antigravity: ${resumeCommand}`);
  }
