...
argument-hint: "[--background|--wait] [--resume|--fresh] [--model <model>] [--effort <none|minimal|low|medium|high>] [what Antigravity should investigate, solve, or continue]"
...
- Leave `--effort` unset unless the user explicitly asks for a specific reasoning effort. Note that `--effort` is currently accepted but ignored by the Antigravity backend, so it has no effect on the run.
- Leave the model unset unless the user explicitly asks for one. If they ask for a specific model, pass its real Antigravity model name through verbatim with `--model`. Users can list available models with `agy models`. Do not invent or map model names.
- Leave `--resume` and `--fresh` in the forwarded request. The subagent handles that routing when it builds the `task` command.
- If the helper reports that Antigravity is missing or unauthenticated, stop and tell the user to run `/agy:setup`.
